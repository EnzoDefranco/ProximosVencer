<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs md:text-sm text-gray-500">
        Rango: {{ \Carbon\Carbon::parse($desde)->format('d/m/Y') }} —
  ...
  <div class="mx-auto max-w-7xl p-4 md:p-8 space-y-4">
    ...
    {{-- Filtros --}}
    <form method="GET" action="{{ route('vencidos.index') }}" id="form-filtros"
      ...
      <div class="min-w-[220px]">
        ...
        <input type="text" name="q" value="{{ $q }}" placeholder="Código o descripción…"
      ...
      <div class="flex items-center gap-2 ml-auto">
        ...
        {{-- Botón imprimir --}}
        <button type="button" onclick="enviarVencidosImprimir()"
            ...
            <tr class="border-t border-gray-100 hover:bg-blue-50/40 transition">
              ...
              @can('eliminar-vencidos')
                <td class="p-3 text-center">
                  <form action="{{ route('vencidos.destroy', $row->id) }}" method="POST"
                    onsubmit="return confirm('¿Eliminar este artículo de la lista de vencidos?');">
                    @csrf
                    @method('DELETE')
